...
import { getHeadquartersListAPI } from '@/api/headquarters'
...
const init = async () => {
  const [{ data: storeRes }, { data: hqRes }] = await Promise.all([getStoreListAPI(), getHeadquartersListAPI()])
  list.value = storeRes.data || []
  headquartersList.value = hqRes.data || []
}
